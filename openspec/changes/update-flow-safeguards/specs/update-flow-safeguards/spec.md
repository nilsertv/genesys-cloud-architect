# Delta for Update Flow (Safeguards)

Delta against `openspec/specs/update-flow/spec.md`. Adds persisted-baseline
capture, a full-baseline diff gate, and a two-call explicit-confirmation
publish protocol to the existing `update_flow` capability. Confirmed
decisions below come from `state.yaml`'s `open_decisions` and are not
re-litigated here.
...
protocol and the full-baseline diff gate defined above: no single call MAY
