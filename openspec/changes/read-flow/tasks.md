...
- [x] 2.4 `pnpm test` (26/26 verde: 20 existentes + 6 nuevos), `pnpm exec tsc --noEmit` (0 errores), `pnpm run build` (limpio), `pnpm run lint` (limpio, tras `lint:fix` de un solo wrap de línea en el test nuevo).
  - **Deuda técnica conocida (revisión independiente de Slice B, no bloqueante)**: `read-flow.test.ts` cubre `tools/list` y la validación cruzada de `callTool`, pero NO cubre el mapeo `ERROR_KIND_MESSAGES` ni el parseo NDJSON del handler (`read-flow.ts:203-274`) — `design.md`'s "Testing Strategy" pedía explícitamente un mock de `spawn` para esto, y quedó fuera de scope. No hay precedente de mock de `spawn` en este repo (`deploy-flow.ts` tampoco lo tiene), así que sería infraestructura nueva, no una copia rápida. Ningún bug confirmado — la lógica se revisó a mano y se ve correcta, y el path de éxito queda cubierto empíricamente en la tarea 3.4. Aceptado como deuda técnica por decisión explícita del usuario; no forma parte del scope de `read-flow`.
  - **Hardening menor pendiente (mismo review, no bloqueante)**: el path de éxito (`read-flow.ts` ~línea 242-250) no valida que `resultLine.content` exista antes de declarar `success` — si el deploy-runner emitiera `{success:true}` sin `content`, el tool devolvería texto vacío sin `isError`. No es una regresión de este change (mismo patrón débil ya presente en `deploy-flow.ts`); no se corrige acá.
