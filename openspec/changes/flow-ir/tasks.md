...
- [ ] 1.1 Call `find_flow` for `ZZZ-SDD-Test-DoNotUse-UpdateFlow` in org "Calidda" (flowId `69cd3550-0848-4fd7-a5c7-4be615b20ced`), then `fetch-flow-configuration` to get the raw flow-configuration JSON.
- [ ] 1.2 Scrub customer-identifying string values (org name, phone numbers, real queue/user names) from the captured JSON without altering structure or field names; save as `src/mcp-server/tools/__fixtures__/real-calidda-flow.json`.
- [ ] 1.3 From the real fixture, resolve design's Open Questions: confirm/correct `paths` vs `outputs`, the task-reference field name, `menuChoiceList` choice shape, and the `reusable` task-flag key. Note actual field names inline as a short comment at the top of `flow-ir-parser.ts` once written (task 2.1), or here if writing tasks first.
...
- [ ] 1.5 **Empirical verification** — confirm the saved fixture round-trips as valid JSON (`node -e "require('./src/mcp-server/tools/__fixtures__/real-calidda-flow.json')"` or equivalent) and that no scrubbed value broke `flowSequenceItemList` structure.

## Phase 2: `flow-ir-parser.ts` — `parseFlow` (PR 2, base = PR1 branch)

- [ ] 2.1 RED — `flow-ir-parser.test.ts`: `parseFlow` returns `{ok:false}` (not throw) for missing/non-array `flowSequenceItemList` (spec: parseFlow Contract).
- [ ] 2.2 GREEN — `flow-ir-parser.ts`: precondition guard + `{ok:false, error}` return path; `enumerateRawActions()` primitive (tolerant, never throws, walks `flowSequenceItemList[].actionList[]` + inline `menuChoiceList[].action`).
- [ ] 2.3 RED — synthetic fixtures `warnings/missing-duplicate-action-id.json`; tests asserting `MISSING_ACTION_ID` and `DUPLICATE_ACTION_ID` (spec: Warning Code Emission).
- [ ] 2.4 GREEN — graph build step: one `task-start` node per task (`<taskId>::start`), one `action` node per first `actionId` occurrence, drop/warn on missing or duplicate ids.
- [ ] 2.5 RED — synthetic fixture `warnings/unresolved-reference.json`; test asserting `UNRESOLVED_REFERENCE` on an unresolvable task-jump (spec: Warning Code Emission, Node ID Scheme task-start id).
- [ ] 2.6 GREEN — task-jump probe (`raw.task?.id ?? raw.taskId ?? raw.destinationTaskId`), direct edge to `<taskId>::start`, `UNRESOLVED_REFERENCE` on miss.
- [ ] 2.7 RED — synthetic fixture with a `menuChoiceList` task; test asserting each choice becomes a `branch-output` node with the choice's inline action wired as successor (spec: Inline menu-choice action).
- [ ] 2.8 GREEN — menu-choice expansion step per design step 3b.
- [ ] 2.9 RED — synthetic fixture `warnings/unresolved-intent-fanout.json`; test asserting `UNRESOLVED_INTENT_FANOUT` and `ir.reachabilityIsComplete === false`.
- [ ] 2.10 GREEN — `INTENT_FANOUT_ACTION_TYPES` seeded allowlist + intent-fan-out exclusion step (design step 3c).
- [ ] 2.11 RED — synthetic fixtures `warnings/disabled-branch.json`, `warnings/dropped-edge.json`; tests asserting `DISABLED_BRANCH` (edges stay in graph) and `DROPPED_EDGE` (edge discarded) (spec: DISABLED_BRANCH, DROPPED_EDGE scenarios).
- [ ] 2.12 GREEN — generic outputs probe (`raw.paths ?? raw.outputs`, else `raw.nextAction ?? raw.nextActionId` fall-through), `branch-output` node creation (`<actionId>::<outputId>`), `DISABLED_BRANCH`/`DROPPED_EDGE` emission (design step 3d).
